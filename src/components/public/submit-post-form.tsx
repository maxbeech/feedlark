...
export function SubmitPostForm({ boardId }: { boardId: string }) {
  ...
  if (!open) {
    return (
      <Card className="p-4">
        <button onClick={() => setOpen(true)} className="w-full rounded-xl bg-slate-50 px-4 py-3 text-left text-sm text-ink-muted hover:bg-slate-100">
          💡 Suggest a feature or report an issue…
        </button>
        {state.ok && <p className="mt-2 text-center text-sm text-emerald-700">Thanks! Your idea is posted.</p>}
      </Card>
    );
  }
    ...
    <Card className="p-5">
      <form ref={formRef} action={action} className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Input name="authorEmail" type="email" placeholder="Email — notify me when shipped" />
        </div>
